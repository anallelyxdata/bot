...
{
    public class CambioTurnoDialog: ComponentDialog
    {
        #region Variables
        ...
        #endregion
        public CambioTurnoDialog(string dialogId, StateService stateService) : base(dialogId)
        {
        ...
        private void InitializeWaterfallDialog()
        {
            ...
            AddDialog(new WaterfallDialog($"{nameof(CambioTurnoDialog)}.mainFlow", waterfallSteps));
            AddDialog(new TextPrompt($"{nameof(CambioTurnoDialog)}.confirmarReposicion"));
            AddDialog(new TextPrompt($"{nameof(UniformesDialog)}.confirmarRegreso"));

            //Indicamos con cual subdialogo comenzar
            InitialDialogId = $"{nameof(CambioTurnoDialog)}.mainFlow";
        }
        ...
        private async Task<DialogTurnResult> ConfirmarReposicionStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            var options = await stepContext.PromptAsync($"{nameof(CambioTurnoDialog)}.confirmarReposicion",
                new PromptOptions
                {
        ...
        private async Task<DialogTurnResult> GeneracionTicketStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if ((string)stepContext.Values["confirmarReposicion"] == "SI")
            {
                ...
                var detallesReposicion = "Cambio de turno";
                try
                {
        ...
        {
            var reply = MessageFactory.Text($"¿Confirmas tu solicitud para cambiar tu turno?");
